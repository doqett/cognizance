    ...
    <section className="py-28">
      <div className="container mx-auto px-4 md:px-8">
        <div className="max-w-md">
          <h1 className="text-2xl font-extrabold sm:text-3xl">Open Positions</h1>
          <p className="text-muted-foreground text-sm mt-2">
            We&apos;re currently looking talent software engineers, and designers to help us in our
            missions and to grow up.
          </p>
        </div>
        <ul className="mt-12 space-y-3 w-fit">
          {openings.map((item, idx) => (
            <>
              <li
                key={idx}
                className="px-4 py-5 duration-200 hover:bg-accent"
              >
                <a href={item.path} className="space-y-3">
                  <div className="flex items-center gap-x-3">
                    <div className="flex items-center justify-center">{item.company_icon}</div>
                    <div>
                      <span className="block text-sm text-blue-500 font-medium">
                        {item.company_name}
                      </span>
                      <h3 className="text-basefont-semibold mt-1">{item.job_title}</h3>
                    </div>
                  </div>
                  <p className="text-muted-foreground sm:text-sm">{item.job_description}</p>
